...
listWorker.postMessage({message: 'get-list', model: 'chirpity', listType: 'location', currentLabels: [list], useWeek: true, lat: 52.0, lon: -0.5, week: 40, threshold: 0.01 })
        ...
        if (listType === "everything") {
            ...
            for (let i = 0; i < mdata_probs.length; i++) {
                ...
                if (mdata_probs[index] < threshold) {
                    ...
                    foundIndices.forEach(index => {
                        ...
                        //if (! ['Dotterel', 'Stone-curlew', 'Spotted Crake'].some(currentLabels[index])) BLOCKED_IDS.push(index)
            ...
            */
            ...
            for (let i = 0; i < currentLabels.length; i++) {
                ...
                const latin = getFirstElement(item);
                const callBehaviourID = ACTIVITY_INDEX[latin] && ACTIVITY_INDEX[latin][0];
                
                if (callBehaviourID && callBehaviourID !== 1  && findSubstringIndex(NOT_BIRDS, latin) === -1) {
                    
                    // find the index of these birds in the current labels:
                    const foundIndices = currentLabels.reduce((indices, element, index) => {
                        if (element.includes(latin)) indices.push(index); 
                        return indices;
                    }, []);
                    foundIndices.forEach(index => {
                        includedIDset.add(index)
                        DEBUG && console.log('Including: ', index, 'name', currentLabels[index] )
                    })
                }
            }
            //includedIDs.sort((a, b) => a - b);
            if (localBirdsOnly){
                ...
                local_ids.sort((a, b) => a - b);
                // Create a list of indices that appear in both lists
                includedIDs = getCommonElements(includedIDset, local_ids);
            } else {
                    ...
                    if (indexes.length) {
                        ...
                        if (indexes.length > 1){
                            for (let idx of indexes) {
                                // Extract word in brackets from current index of currentLabels
                                const wordInBrackets = currentLabels[idx].match(/\((.*?)\)/);
                                if (wordInBrackets) {
            ...
            includedIDs = currentLabels.map((label, index) => {
...
function getCommonElements(largeSet, smallList){
...
//Optimised list search
function findSubstringIndex(sortedArray, searchString) {
    let left = 0;
    ...
    while (left <= right) {
        ...
        // Check if the middle value contains the substring
        if (midValue.includes(searchString)) {
            return mid; // Found the substring at index mid
        }

        // Since the array is alphabetically sorted, we adjust the search range
        if (midValue < searchString) {
